...
### --- Objective function --- ###
si.lf <- function(theta, bw = length(y)**(-1/5), y, x, xi, lambda_lasso, a_coste = 0.047 * 0.05, b_coste = 10, c_coste = 1) {
  k <- ncol(x) 
  n <- nrow(x)
  ...
  xi <- -c_coste*y*w + (1-y)*w*a_coste + b_coste # loss function weights
  
  # --- Average expected cost (AEC) --- #
  loss <- sum(c_coste*y*w + p*xi) / n + lambda_lasso*sum(abs(theta))  # CS proposal
  return( -loss ) 
